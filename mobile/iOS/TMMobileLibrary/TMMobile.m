...
static BOOL isTelstraMobile = false;  //False=not on mobile network, True=On Telstra Mobile Network

static NSString *TM_VERSION = @"iOS-20200210-01"; // TM Framework version. Needs to be updated when the framework is updated.
static NSString *lastSection = @"";
static BOOL disableSectionReports = false;
...
disableSectionReports:(BOOL)sectionReports {
...
+(void) tmTrackStateInternal:(NSString *)state
    ...
    NSString *appsection = [dataToSend objectForKey:@"pageinfo.appsection"];
    //populate custom context data
    if (data) {
        [dataToSend addEntriesFromDictionary:data];
    }
    
    if (!disableSectionReports || appsection == nil) {
         [super trackState:pageNameStr data:dataToSend];
         lastSection = @"";
     }
    else if(appsection.length > 0 && ! [lastSection isEqualToString:appsection ]) {
         lastSection = [dataToSend objectForKey:@"pageinfo.appsection"];
