...
window.additionalEmbed = function(videoPlayer, ns) 
	{
		...
		//HTML5
		videoPlayer.mb.subscribe(window[videoPlayer.videons].EVENTS.STREAM_PLAYING, 'TelstraMedia', 
		function(eventName) {
			_satellite.notify("[html5->*****"+videoPlayer.videons+"] "+videoPlayer.vmsID+" STREAM_PLAYING-> adfinish:"+videoPlayer.adFinished);
		...
		function(eventName) {
			_satellite.notify("[flash->+++++"+videoPlayer.videons+"] "+videoPlayer.vmsID+" PLAYING-> adfinish:"+videoPlayer.adFinished);
			if (videoPlayer.adFinished == true) {
			trackMediaPlay( videoPlayer.vmsID, typeof videoPlayer.adStartTime !== 'undefined' && videoPlayer.adStartTime > -1 ? videoPlayer.adStartTime : videoPlayer.getPlayheadTime(), videoPlayer );
			videoPlayer.adStartTime = -1;
			}
		...
		function(eventName, arg1, arg2, arg3) {
			_satellite.notify('*****PLAYBACK_READY videoplay start');
			
			var playHeadTime = typeof videoPlayer.getPlayheadTime() !== 'undefined' ? videoPlayer.getPlayheadTime() : arg1 ;
			
			if(typeof arg1 === 'undefined' )
			{
				//fix Ooyala video duration stuff up
				videoPlayer.videoDuration = typeof videoPlayer.item.time !== 'undefined'? videoPlayer.item.time : videoPlayer.getTotalTime()/1e3;
				trackMediaOpen( videoPlayer.vmsID, videoPlayer.videoDuration, videoPlayer ); 
			}
...
/*				videoPlayer.mb.subscribe(window[videoPlayer.videons].EVENTS.PLAYHEAD_TIME_CHANGED, 'TelstraMedia', 
		function(eventName, arg1, arg2, arg3) {
			...
			if(typeof arg1 === 'undefined')
			{
				...
				trackMediaOpen( videoPlayer.vmsID, arg2, videoPlayer ); 
			}
			...
			if( videoPlayer.videoPlaying )
			{
				...
				// {
				// 	_satellite.notify('*****playheadtimebefore videoplay start: ' + playHeadTime + ":" + videoPlayer.videoPlayStart);
				// 	trackMediaClose( videoPlayer.vmsID );
				// 	trackMediaPlay( videoPlayer.vmsID, playHeadTime );				
				// }
				s.eVar64 = '' + Math.floor( playHeadTime / (videoPlayer.videoSecondaryTimingActive ? videoPlayer.videoPrimaryTimingGranularity : videoPlayer.videoPrimaryTimingGranularity) ) * (videoPlayer.videoSecondaryTimingActive ? videoPlayer.videoPrimaryTimingGranularity : videoPlayer.videoPrimaryTimingGranularity);				
			}
		});
*/
		...
		function(eventName, metadata) {
			...
			videoPlayer.vmsID = videoPlayer.metadata.base.VmsId;
